...
import { TrendingUp, Mail, Lock, User, AlertCircle, Sparkles } from "lucide-react";
...
export function Auth() {
  ...
  const handleAuthSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isForgotPassword) {
        ...
        await register(email.trim(), password, displayName.trim() || "Investor");
        sessionStorage.setItem("investiq_just_registered", displayName.trim() || "Investor");
        navigate("/");
      }
    } catch (err: any) {
      setToastMessage({ type: "error", text: err.message || "Authentication failed. Please check your credentials." });
    } finally {
  ...
  const handleDemoLogin = async () => {
    ...
    setIsSubmitting(true);
    try {
      await login("demo.investor@investiq.com", "investiq123");
      navigate("/");
    } catch (err: any) {
      setToastMessage({ type: "error", text: err.message || "Demo login failed" });
    } finally {
      setIsSubmitting(false);
    }
  };
        ...
        {isForgotPassword ? (
          ...
          <form onSubmit={handleAuthSubmit} className="space-y-5">
            ...
            <button
              type="submit"
              disabled={isSubmitting}
              className="w-full h-10 bg-emerald-500 hover:bg-emerald-600 text-white rounded-lg text-sm font-medium transition-colors disabled:opacity-50 cursor-pointer shadow-lg shadow-emerald-500/20 flex items-center justify-center gap-2"
            >
              {isSubmitting ? (
            ...
            {/* Google Authentication & Demo Login */}
            <div className="space-y-2">
              ...
              <button
                ...
                disabled={isSubmitting}
                className="w-full h-9 bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-400 border border-emerald-500/20 rounded-lg text-xs font-semibold transition-all flex items-center justify-center gap-1.5 cursor-pointer disabled:opacity-50"
              >
